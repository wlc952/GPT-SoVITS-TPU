...
class GptSovits:
    def __init__(self, model_path="models"):
        ...
        self.sample_layer_topk = ort.InferenceSession(model_path + '/08_t2s_sample_layer_adjust_k.onnx')
        self.sample_layer_topkp = ort.InferenceSession(model_path + '/08_t2s_sample_layer_adjust_k&p.onnx')
    ...
    def t2s_process(self, phoneme_ids, bert, prompts, top_k=15, top_p=1.0):
        sample_dict = {"top_k": np.int64([top_k])}
        if top_p == 1.0:
            sample_layer = self.sample_layer_topk
        else:
            sample_layer = self.sample_layer_topkp
            sample_dict["top_p"] = np.float32([top_p])
        ...
        samples, y = sample_layer.run(None, sample_dict)
    ...
    def __call__(self, ref_wav_path, text, prompt_text, top_k=15, top_p=1.0):
        start = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text", type=str, default="此外内容还和芯片后端以及产品硬件前端相呼应。正是像停云小姐这样的节度使往来周旋，请从这里裁剪。请从这里裁剪。") # 四个标点，85字以内。
